...
const ConfidenceBar = ({ confidence, ...props }: ConfidenceBarProps) => {
  ...
  return (
    <span {...props} className={`flex gap-1 ${props.className} text-sm items-center`}>Confidence:
      {Array.from({ length: 4 }).map((_, i) => (
        <span key={i} className={` ${level > i ? "bg-primary border-primary" : "bg-background border-green-900 border"} w-3 h-3 rounded-sm border-2`} />
      ))}
    </span>
  )
}
